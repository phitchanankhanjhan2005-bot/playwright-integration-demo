  ...
  try {
    await Promise.all([
      ...
      (async () => {
        ...
        await expect(topPage.locator('.inventory_list')).toBeVisible();
      ...
      (async () => {
        ...
        // B REAL -> E REAL
        await bottomPage.locator('[data-test="add-to-cart-sauce-labs-backpack"]').click();
        await expect(bottomPage.locator('.shopping_cart_badge')).toHaveText('1');

        // E -> F REAL
        await bottomPage.locator('.shopping_cart_link').click();
        await expect(bottomPage.locator('.inventory_item_name')).toHaveText('Sauce Labs Backpack');
      })(),
    ]);
  } finally {
    await Promise.all([
